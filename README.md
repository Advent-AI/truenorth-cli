...
- **HIP-4 Outcome Markets** — Composite trader view of Hyperliquid prediction markets with pre-computed insights (flow, divergence, slippage, no-arb)
...
Read-only access to [Hyperliquid HIP-4](https://hyperliquid.gitbook.io/hyperliquid-docs/hyperliquid-improvement-proposals-hips/hip-4-outcome-markets) outcome / prediction markets.

| Command | Description |
|---------|-------------|
| `tn hip4 markets` | List active outcome markets |
| `tn hip4 details <id> [--interval 5m] [--lookback 1h] [--trade-sample 50]` | Composite trader view: quote + depth + flow + momentum + insights in one call |

```bash
tn hip4 markets                            # See what's live
tn hip4 details 1                          # Full trader snapshot for outcome #1
tn hip4 details 1 --lookback 4h            # Wider momentum window
tn hip4 details 1 --json                   # Raw JSON for agents
```

The `details` response includes pre-computed insights such as flow direction (buy/sell pressure), divergence detection (price moving against recent flow), no-arb sanity check, slippage at trader sizes ($100/$500/$1K/$5K/$10K), and TTL — so an agent can decide direction, sizing, and timing in a single call.
